...
public:
  SDFMap() {}
  SDFMap(ros::NodeHandle& nh) : node_(nh) {}
  ~SDFMap() {}
  ...
  // QHB：用来测试topo_rpm
  void initMapFromCostMap();
  bool voro_plan(Eigen::Vector3d start, Eigen::Vector3d goal, double start_yaw = 0.0)
  {
    return voronoi_layer_->plan(start, goal, start_yaw);
  }
