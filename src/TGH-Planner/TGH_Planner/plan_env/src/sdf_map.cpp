...
void SDFMap::initMap() {
  ...
  if (mp_.perception_data_type_ == DEPTH_IMAGE) {
    ...
    // 这里的odom_sub_是订阅的传感器的odom，而不是机器人的odom
    // cloud_sub_.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(node_, "/sdf_map/perception_data", 10));
    // odom_sub_.reset(new message_filters::Subscriber<nav_msgs::Odometry>(node_, "/sdf_map/perception_odom", 100));
...
void SDFMap::CostmapCallback(const nav_msgs::OccupancyGridConstPtr &map_msg)
{
  ...
  double ratio = map_msg->info.resolution / mp_.resolution_;
  int resolution_scale = (ratio > 1.0) ? static_cast<int>(round(ratio)) : static_cast<int>(ceil(1.0 / ratio));
  resolution_scale = std::max(1, resolution_scale);
  ...
  if(mp_.need_map_2D_) inf_pts.resize(pow(2 * inf_step + 1, 2), Eigen::Vector3i(0, 0, 0));
  else inf_pts.resize(pow(2 * inf_step + 1, 3), Eigen::Vector3i(0, 0, 0));
  Eigen::Vector3i inf_pt;
  vector<Eigen::Vector2i> obs_pts;
  // 读取costmap的信息
    ...
    for(int h = 0; h < map_msg->info.height; ++h)
    {
      ...
      else if(map_msg->data[map_idx] != 0)
      {
        for(int i = 0; i < resolution_scale; ++i)
        {
          for(int j = 0; j < resolution_scale; ++j)
          {
            obs_pts.push_back(Eigen::Vector2i((int)(resolution_scale * w + i), (int)(resolution_scale * h + j)));      
          }
        }
      }
    }
  ...
  {
    inflatePoint(Eigen::Vector3i(pt.x(), pt.y(), 0.5), inf_step, inf_pts);
...
int SDFMap::setCacheOccupancy2D(Eigen::Vector3d pos, int occ, bool check_occ) {
  ...
  if (occ == 1) 
  {
    md_.count_hit_and_miss_2D_[idx_ctns] = 0;
    md_.count_hit_2D_[idx_ctns] += 1;
  }
...
void SDFMap::pointcloudToLaserScan(
    ...
    float inf_epsilon)
 {
    ...
    // 计算scan的size
    uint32_t ranges_size = static_cast<uint32_t>(std::floor((angle_max - angle_min) / angle_increment) + 1.0);
    angle_max = angle_min + (ranges_size - 1) * angle_increment;
    ...
    for (int i = 0; i < md_.proj_points_cnt; ++i) {
        ...
        float fidx = (angle - angle_min) / angle_increment;
      ...
      return;
      // 将scan里面的点云转换到world系下然后发布出来
      // 可能存在恢复点云时的像素误差，但是整体应该是没有偏移的。是稠密化地方的问题？
      pcl::PointCloud<pcl::PointXYZ> cloud_use_for_scan;
      pcl::PointXYZ pt;
      for (int i = 0; i < ranges_size; ++i)
      {
        double angle = md_.bearing_[i];
        if(std::isnan(angle)) angle = angle_min + i * angle_increment;
        double local_x = scan_msg.ranges[i] * cos(angle);
        double local_y = scan_msg.ranges[i] * sin(angle);
        pt.x = local_x;
        pt.y = local_y;
        pt.z = 0.0;
        cloud_use_for_scan.push_back(pt);
        // std::cout << "scan pt: " << pt_local.transpose() << std::
      }
      Eigen::Matrix3d scan_r = md_.scan_q_.toRotationMatrix(); 
      Eigen::Vector3d scan_t = md_.scan_pos_; 
      Eigen::Affine3d tf_scan_in_world;
      tf_scan_in_world.linear() = scan_r;
      tf_scan_in_world.translation() = scan_t;
      // 设置点云的元信息
      cloud_use_for_scan.width = cloud_use_for_scan.points.size();
      cloud_use_for_scan.height = 1;
      cloud_use_for_scan.is_dense = true;
      pcl::transformPointCloud(cloud_use_for_scan, cloud_use_for_scan, tf_scan_in_world);
      // 转换为 ROS 消息
      sensor_msgs::PointCloud2 cloud_msg;
      pcl::toROSMsg(cloud_use_for_scan, cloud_msg);

      // 设置消息头
      cloud_msg.header.frame_id = "world";
      cloud_msg.header.stamp = ros::Time::now();

      static ros::Publisher proj_points_pub = node_.advertise<sensor_msgs::PointCloud2>("/sdf_map/cloud_use_for_scan", 1, true);
      proj_points_pub.publish(cloud_msg);      
...
void SDFMap::raycastProcess() {
  ...
  std::sort(md_.proj_points_.begin(), md_.proj_points_.begin() + md_.proj_points_cnt,
  [&camera_pos](const Eigen::Vector3d& a, const Eigen::Vector3d& b) {
      return (a - camera_pos).squaredNorm() > (b - camera_pos).squaredNorm();
  });
  ...
  for (int i = 0; i < md_.proj_points_cnt; ++i) {
    ...
    raycaster.setInput((pt_w - 1 * dir * mp_.resolution_) / mp_.resolution_, md_.camera_pos_ / mp_.resolution_);
                               ...
                               double half_fov, double min_dist, double max_dist)
{
  ...
  float last_range;
  bool has_last = false;
  const int N = static_cast<int>(scan_pt.ranges.size());
  auto idx_from_angle = [&](double a)->int {
    return static_cast<int>(std::floor((a - scan_pt.angle_min) / scan_pt.angle_increment));
  };
  int start_idx = std::max(0,   idx_from_angle(-half_fov));
  int end_idx   = std::min(N,   idx_from_angle( half_fov) + 1);
  ...
  for (size_t i = start_idx; i < end_idx; ++i) {
    ...
    float angle = md_.bearing_[i];
    bool beyond_range = false;
    if (std::isnan(angle))
    {
      beyond_range = true;
      angle = scan_pt.angle_min + i * scan_pt.angle_increment;
    }
    // 激光点在scan_frame坐标系下
    ...
    // 如果距离太大，插值
    if (dist > max_dist) {
      if ((range - scan_pt.range_max) * (last_range - scan_pt.range_max) < 0) {
        dense_points.push_back(pt);
        last_pt = pt;
        last_range = range;
        continue;
      ...
      // TODO: 这里还可以加入，v1和其他点的夹角判断。
      if (dist_to_line < 0.2 || dist > 6 * max_dist) {
        // 如果两点共线，直接跳过当前点
        dense_points.push_back(pt);
        last_pt = pt;
        last_range = range;
...
void SDFMap::raycastProcess2D() {
  ...
  // ---------------------- //
  std::sort(md_.scan_pt_refined_.begin(), md_.scan_pt_refined_.end(),
    [&pos](const Eigen::Vector3d& a, const Eigen::Vector3d& b) {
        return (a - pos).squaredNorm() > (b - pos).squaredNorm();
    });
  ...
  // cv::Mat hit_miss_image(height, width, CV_8UC1, cv::Scalar(0)); 
  // cv::Mat hit_image(height, width, CV_8UC1, cv::Scalar(0)); 
  // cv::Mat miss_image(height, width, CV_8UC1, cv::Scalar(0)); 
  // // 遍历每个栅格
  // for (int x = 0; x < width; ++x) {
  //   for (int y = 0; y < height; ++y) {
  //     int idx = toAddress2D(x, y);

  //     // 获取 count_hit_and_miss_2D_ 和 count_hit_2D_ 的值
  //     int hit_and_miss = md_.count_hit_and_miss_2D_[idx];
  //     int hit = md_.count_hit_2D_[idx];

  //     // 映射到颜色值（归一化到 0-255）
  //     uchar hit_color = static_cast<uchar>(std::min(hit * 255 / 10, 255)); // 假设最大值为 10
  //     uchar hit_miss_color = static_cast<uchar>(std::min((hit_and_miss) * 255 / 10, 255));
  //     uchar miss_color = static_cast<uchar>(std::min((hit_and_miss - hit) * 255 / 10, 255));
  //     // 设置图像像素值
  //     hit_miss_image.at<uchar>(height - y - 1, x) = (uchar)hit_miss_color;
  //     hit_image.at<uchar>(height - y - 1, x) = (uchar)hit_color;
  //     miss_image.at<uchar>(height - y - 1, x) = (uchar)miss_color;
  //   }
  // }

  // // 保存图像到文件
  // cv::imwrite("/home/bhrqhb/catkin_TGH_new/src/TGH_Planner/Utils/test/hit_miss_image.png", hit_miss_image);
  // cv::imwrite("/home/bhrqhb/catkin_TGH_new/src/TGH_Planner/Utils/test/hit_image.png", hit_image);
  // cv::imwrite("/home/bhrqhb/catkin_TGH_new/src/TGH_Planner/Utils/test/miss_image.png", miss_image);
  ...
  while (!md_.cache_voxel_2D_.empty()) {
    ...
    //如果是占据，则log_odds_update大于0；否则小于0
    double log_odds_update =
        md_.count_hit_2D_[idx_ctns] >= md_.count_hit_and_miss_2D_[idx_ctns] - 1 * md_.count_hit_2D_[idx_ctns] ? //判断这个voxel是占据 or free
        mp_.prob_hit_log_ :
        mp_.prob_miss_log_;
...
void SDFMap::clearAndInflateLocalMap2D() {
  ...
  int inf_step = ceil(-0.01 / mp_.resolution_);
  // int inf_step_z = 1;
  vector<Eigen::Vector3i> inf_pts;
  if(inf_step == 0) inf_pts.resize(1);
  else if(mp_.need_map_2D_) inf_pts.resize(pow(2 * inf_step + 1, 2), Eigen::Vector3i(0, 0, 0));
  else inf_pts.resize(pow(2 * inf_step + 1, 3), Eigen::Vector3i(0, 0, 0));
  Eigen::Vector3i inf_pt;
...
// 现在我不要再单独更新esdf地图了，而是在更新占据栅格地图之后，调用voronoi_layer的updateESDF函数
// 然后将结果存储在md_.distance_buffer_all_2D_里面
...
void SDFMap::cloudOdomCallback(const sensor_msgs::PointCloud2ConstPtr& lidar_pt,
  ...
  // 用 odom 的位姿作为 lidar 在 world 下的位姿
  Eigen::Vector3d lidar_position;
  lidar_position.x() = odom->pose.pose.position.x;
  lidar_position.y() = odom->pose.pose.position.y;
  lidar_position.z() = odom->pose.pose.position.z;
  ...
  // 用传感器在 base_link 下的相对位姿做变换（如有需要）
  Eigen::Vector3d lidar_position_on_base;
  Eigen::Quaterniond lidar_orientation_on_base;
  while (ros::ok() && !getTransform(mp_.lidar_link_, "base_link", ros::Time(0), lidar_position_on_base, lidar_orientation_on_base)) {
      ROS_WARN("Waiting for TF: %s -> %s ...", mp_.lidar_link_.c_str(), mp_.frame_id_.c_str());
      ros::Duration(0.1).sleep();
  }
  mp_.sensor_on_base_pos_ = lidar_position_on_base;
  mp_.sensor_on_base_q_ = lidar_orientation_on_base;
  ...
  Eigen::Vector3d p3d_world, p3d_local;
  int inf_step = ceil(mp_.obstacles_inflation_ / mp_.resolution_);
  ...
  for (size_t i = 0; i < latest_cloud_world.points.size(); ++i) {
    ...
    if(pt_dis > (mp_.max_ray_length_ + 0.1)) {
      p3d_local = (p3d_local) / pt_dis * (mp_.max_ray_length_ + 0.1);
...
void SDFMap::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& lidar_pt) {
  ...
  // 将点云从 ROS 消息转换为 PCL 格式
  pcl::PointCloud<pcl::PointXYZ> cloud_lidar, cloud_world, cloud_projected;
  pcl::fromROSMsg(*lidar_pt, cloud_lidar);
  md_.sensor_time_ = lidar_pt->header.stamp;
  // 查询 lidar_link 到 world 的变换
  Eigen::Vector3d T_w_l_p;
  Eigen::Quaterniond T_w_l_r;
  if (!getTransform(lidar_pt->header.frame_id, mp_.frame_id_, md_.sensor_time_, T_w_l_p, T_w_l_r)) {
    ROS_WARN("Failed to get lidar pose for cloudCallback.");
    return;
  }

  Eigen::Affine3d T_w_l = Eigen::Translation3d(T_w_l_p) * T_w_l_r;
  pcl::transformPointCloud(cloud_lidar, cloud_world, T_w_l);

  Eigen::Vector3d T_b_l_p;
  Eigen::Quaterniond T_b_l_r;
  while (ros::ok() && !getTransform(mp_.lidar_link_, "base_link", md_.sensor_time_, T_b_l_p, T_b_l_r)) {
      ROS_WARN("Waiting for TF: %s -> %s ...", mp_.lidar_link_.c_str(), "base_link");
      ros::Duration(0.1).sleep();
  }

  Eigen::Affine3d T_b_l = Eigen::Translation3d(T_b_l_p) * T_b_l_r;


  // 构造目标投影坐标系的旋转矩阵
  Eigen::Matrix3d R_b_l = T_b_l_r.toRotationMatrix();
  Eigen::Vector3d z_axis(0, 0, 1);  // base_link 的 z 轴
  Eigen::Vector3d lidar_z_in_base = R_b_l.col(2);
  Eigen::Quaterniond R_align_q = Eigen::Quaterniond::FromTwoVectors(lidar_z_in_base, Eigen::Vector3d::UnitZ());
  Eigen::Matrix3d R_align = R_align_q.toRotationMatrix();
  Eigen::Matrix3d R_P_L = R_align * R_b_l;
  Eigen::Vector3d p0_base(T_b_l_p.x(), T_b_l_p.y(), 0.0);
  Eigen::Vector3d t_P_L = R_align * (T_b_l_p - p0_base);

  Eigen::Affine3d T_P_L = Eigen::Affine3d::Identity();
  T_P_L.linear() = R_P_L;
  T_P_L.translation() = t_P_L;
  pcl::transformPointCloud(cloud_lidar, cloud_projected, T_P_L);

  Eigen::Affine3d T_W_P = T_w_l * T_P_L.inverse();

  md_.scan_q_   = Eigen::Quaterniond(T_W_P.rotation());
  md_.scan_pos_ = T_W_P.translation();
  mp_.sensor_on_base_pos_ = T_b_l_p;
  mp_.sensor_on_base_q_ = T_b_l_r;
  md_.camera_pos_ = T_w_l_p;
  md_.camera_q_ = T_w_l_r;
  ...
  // 如果点云为空，直接返回
  if (cloud_world.points.size() == 0) return;
  ...
  double pt_angle;
  // 遍历点云
  for (size_t i = 0; i < cloud_world.points.size(); ++i) {
    // 获取点云中的点
    pt_world = cloud_world.points[i];
    pt_local = cloud_projected.points[i];

    float pt_angle = atan2(pt_local.y, pt_local.x);
    // 归一化到[-pi, pi]
    while (pt_angle > M_PI) pt_angle -= 2 * M_PI;
    while (pt_angle < -M_PI) pt_angle += 2 * M_PI;

    // 只保留[-halfFOV, halfFOV]范围内的点
    if (pt_angle <= -mp_.half_fov_ + 1e-3 || pt_angle >= mp_.half_fov_ - 1e-3)
        continue; 
    p3d_local.x() = pt_local.x;
    p3d_local.y() = pt_local.y;
    p3d_local.z() = pt_local.z;
  ...
  // 这说明这样得到的点云是正确的？
  // {
  //   pcl::PointCloud<pcl::PointXYZ> cloud_base_link;
  //   pcl::PointXYZ pt;
  //   for(int i = 0; i < md_.proj_points_cnt; ++i)
  //   {
  //     pt.x = md_.proj_points_local_[i](0);
  //     pt.y = md_.proj_points_local_[i](1);
  //     pt.z = md_.proj_points_local_[i](2);
  //     cloud_base_link.points.push_back(pt);
  //   }
  //   // 设置点云的元信息
  //   cloud_base_link.width = cloud_base_link.points.size();
  //   cloud_base_link.height = 1;
  //   cloud_base_link.is_dense = true;
  //   pcl::transformPointCloud(cloud_base_link, cloud_base_link, tf_scan_in_world);
  //   // 转换为 ROS 消息
  //   sensor_msgs::PointCloud2 cloud_msg;
  //   pcl::toROSMsg(cloud_base_link, cloud_msg);

  //   // 设置消息头
  //   cloud_msg.header.frame_id = "world";
  //   cloud_msg.header.stamp = ros::Time::now();

  //   static ros::Publisher proj_points_pub = node_.advertise<sensor_msgs::PointCloud2>("/sdf_map/base_link_pts", 1, true);
  //   proj_points_pub.publish(cloud_msg);
  // }
...
void SDFMap::publishDepth() {
  ...
  // Eigen::Matrix3d scan_r = md_.scan_q_.toRotationMatrix(); // 相机旋转矩阵的逆
  // Eigen::Vector3d scan_t = md_.scan_pos_; // 相机的平移向量
  // Eigen::Affine3d tf_scan_in_world;
  // tf_scan_in_world.linear() = scan_r;
  // tf_scan_in_world.translation() = scan_t;
